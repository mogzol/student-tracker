...
export default function RequireLogin(props: React.PropsWithChildren<{}>) {
  ...
  if (!googleContext.ready || googleContext.signingIn || dataContext.loading) {
    return <Spinner />;
  }
  ...
  if (!googleContext.signedIn) {
    return (
      <Modal className="login" width="narrow">
        ...
        <div className="text">
          Student Communication Tracker uses your Google Drive account to store data. Please sign in
          with Google using the button below.
        </div>
        <Button text="Sign In with Google" onClick={googleContext.signIn} />
        <div className="disclaimer">
          This will only gives the app access to its own data. Other data in your drive will not be
          accessed or modified. No information about you will be stored.
        </div>
      </Modal>
    );
  }
